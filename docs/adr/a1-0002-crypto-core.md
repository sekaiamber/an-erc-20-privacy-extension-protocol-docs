...
4. **公开输入打包**：标量打包进 3 个字，点只公开 x 坐标、y 由"在曲线上 + 奇偶位"在电路内绑定；transfer 电路公开输入 25 → 15，验证 gas 实测 386k → 316k。（原方案"电路内 SHA-256 压缩为 1 个输入"于 2026-09-24 原型阶段否决：需 ~40 万约束，证明时间不可接受。）
